...
const ExploreScreen = ({ navigation, route }) => {
  ...
  useEffect(()=>{
    getData();
    patientdb.collection("upcomingdoctors").get().then((querySnapshot) => {
      // querySnapshot.forEach((doc) => {
          // alert(doc.data().id);
      // })
      alert(querySnapshot.size);
      // for (const docSnapshot of querySnapshot.docs) {
        // alert(docSnapshot.id);       // THIS WILL PRINT THE DOC ID
      // }
    })
  ...
  async function getData(){
    // patientdb.collection("upcomingdoctors").get();
    // for (const docSnapshot of querySnapshot.docs) {
    ...
    let doctors=[];
    patientdb.collection("doctors").get().then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
          doctors.push({
            name:doc.data().name,
    ...
    .catch((error) => {
        console.log("Error getting doctors documents: ", error);
    });
