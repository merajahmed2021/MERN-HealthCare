...
const HomeScreen = ({ navigation, route }) => {
  ...
  async function getData(){
    let blogs=[];
    let doctors=[];
    patientdb.collection('doctors').doc("123456789").collection("blogs").limit(3).get().then((querySnapshot) => {
    ...
    patientdb.collection("doctors").limit(3).get().then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
          // console.log(doc.id, " => ", doc.data());
          doctors.push({
            name:doc.data().name,
          ...
          <Col>
            <TouchableOpacity   onPress={() => navigation.navigate("CategoryDoctor",
            {
              ...
              <Card style={styles.center}>
                <Avatar.Image  size={60}  source={require("../images/service-7.png")} style={styles.categoryimg1}/>
                <View>
