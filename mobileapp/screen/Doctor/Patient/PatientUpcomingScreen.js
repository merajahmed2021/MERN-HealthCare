...
const AllPatientScreen = () => {
  ...
  async function getData(){
    var patientsarray=[];
    patientdb.collection("patients").doc("Akhlakh Ahmed").collection("Upcoming").orderBy("currenttime").get().then((querySnapshot) => {
    ...
    return (
      <View>
          <View> 
            <Card style={{margin:10,padding:15,marginBottom:0}}> 
              <Title style={{textAlign:'center'}}>Upcoming Patient's Appointment </Title>
            </Card>
          </View>
          <ScrollView>
...

              {
                ...
                :
                  ...
                  <ScrollView key={i} horizontal  scrollEventThrottle={1000}  showsHorizontalScrollIndicator={false}>
                    <DataTable.Row>
                      <DataTable.Cell  style={{paddingRight:45}} >{i+1} </DataTable.Cell>
