...
import { TextInput,Button,Text,Card ,Caption,Title,Subheading,Provider, Menu, Divider} from 'react-native-paper';
...
const RegistrationformScreen = ({navigation,route}) => {
  ...
  const [experincesCategory, setexperincesCategory] = React.useState(' ');
  // console.log(experincesCategory);
  ...
  function selectExperiences(value){
    setexperincesCategory(value);
    setVisible(false);
  }
  ...
  const  uploadImage=async (uri,imageName)=>{
    ...
    (snapshot) => {
      ...
      () => {
        storageRef.child(`doctorprofile/${imageName}`)
  ...
  function handlesubmit(){
    patientdb.collection("upcomingdoctors").doc(userauth.currentUser.uid).collection("details").add({
      name:textname,
      experinces:experinces,
      experincecategory:experincesCategory,
      ...
      navigation.navigate("DoctorRegistertationfinal");
     }).catch((error)=>{
                ...
                <View style={{padding:20}}>
                  ...
                  <View style={{paddingTop: 50,flexDirection: 'row',justifyContent: 'center'}}>
                    <Menu  visible={visible}  onDismiss={closeMenu}
                      anchor={<Button onPress={openMenu} uppercase={false}  mode="outlined">Select category of experinces</Button>}>
                      {
                        category.map((row) => (
                          <Menu.Item onPress={() => { selectExperiences(row.value)}} title={row.name} />
                        ))
                      }
                    </Menu>
                  </View>
                  ...
                  {experincesCategory!=" "?
                     <View style={{marginTop:30}}>
                        <Caption style={{textAlign:'center',fontSize:16}}>Experienced in {experincesCategory}</Caption>
                     </View>
                     :null
                  }
                  ...
                  <View>
                      <TextInput  mode="outlined" label="About you" style={{marginTop:20}}
                         value={about}  multiline={true}  numberOfLines={10}
                  ...
                  <View style={{marginTop:30,paddingBottom:30}}>
                    ...
                    <Button mode="contained"
                     onPress={handlesubmit} style={{marginTop:20,paddingVertical:5,borderRadius:25}}
                   ...
                   category.map((row) => (
                     <View style={{flexDirection:'row',alignContent:'space-between',marginLeft:20}}>
                       <RadioButton value={row.value}  color="#694fad" name={row.value}
                         status={ experincesCategory === row.value ? 'experincesCategory' : 'unexperincesCategory' }
